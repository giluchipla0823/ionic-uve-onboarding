...
@Directive({
  selector: '[myCustomParallaxHeader]',
})
export class MyCustomParallaxHeaderDirective implements AfterContentInit {
    ...
    this.innerScroll.addEventListener('scroll', (_event) => {
      if (!this.ticking) {
        window.requestAnimationFrame(() => {
          ...
          console.log({ scrollTop: this.innerScroll.scrollTop });

          if (this.innerScroll.scrollTop + 56 >= this.height) {
            // this.renderer.setStyle(
            //   this.toolbarContainer,
            //   'align-items',
            //   'center'
            ...
            //   'baseline'
            // );
            // this.renderer.setStyle(this.toolbarContainer, 'align-items', 'center');
          }
        });
      }
      this.ticking = true;
